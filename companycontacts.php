...
if(isset($_GET['cid']) && ($_SESSION['role']=='ADM'||$_SESSION['role']=='COH'||$_SESSION['role']=='BRH')){
	$cid = $_GET['cid'];
    $pre = mysqli_query($connection, "SELECT branch FROM companies WHERE companyid=(SELECT company FROM companycontacts WHERE contactid='$cid')");
    $r = mysqli_fetch_array($pre);
    if($_SESSION['role']=='BRH' && $r['branch'] == getbranchbyid($_SESSION['user'], $connection))
        ...
        <section class="wrapper">
            ...
            <div class="col-sm-12">
                <section class="panel">
                    ...
                    <tbody>
					<?php
                        $exec = "SELECT * FROM companycontacts ";
                        if($_SESSION['role']=='BRH'||$_SESSION['role']=='SAE')
                            {
                                $branch = getbranchbyid($_SESSION['user'], $connection);
                                $exec = $exec."WHERE company = (SELECT companyid FROM companies WHERE branch='$branch')";
                            }
